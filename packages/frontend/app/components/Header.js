...
export default function Header() {
  ...
  useEffect(() => {
    const handleScroll = () => setIsActive(window.scrollY > 30);
        ...
        <nav className={isMenuOpen ? 'navbar nav-toggle' : 'navbar'}>
            <ul>
                ...
                <li>
                  ...
                  {user ? (
                    <button onClick={logout} className="btn-login" style={{ background: 'none', border: 'none', color: '#39a7ff', cursor: 'pointer', fontFamily: "'Lilita One', sans-serif", fontSize: '2rem' }}>Logout</button>
                  ) : (
